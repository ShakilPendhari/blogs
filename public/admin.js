...
function populateCategories(categories) { $('category').innerHTML = categories.map(category => `<option value="${escapeHtml(category)}">${escapeHtml(category)}</option>`).join(''); $('filter-category').innerHTML = '<option value="">All categories</option>' + categories.map(category => `<option value="${escapeHtml(category)}">${escapeHtml(category)}</option>`).join(''); $('category-list').innerHTML = categories.map(category => `<span class="category-chip">${escapeHtml(category)}${category === 'Other' ? '' : `<button type="button" class="category-delete" data-category="${escapeHtml(category)}" aria-label="Delete ${escapeHtml(category)}">&times;</button>`}</span>`).join(''); $('category-list').querySelectorAll('.category-delete').forEach(button => button.addEventListener('click', () => deleteCategory(button.dataset.category))); }
async function loadCategories() { const data = await request('/blogs/categories'); populateCategories(data.categories); }
async function deleteCategory(category) { if (!window.confirm(`Delete the ${category} category?`)) return; try { await request(`/blogs/categories/${encodeURIComponent(category)}`, { method: 'DELETE', headers: headers() }); await loadCategories(); message('Category deleted.'); } catch (error) { message(error.message, true); } }
async function uploadCoverImage() { const file = $('coverImageFile').files[0]; if (!file) return $('coverImage').value || undefined; const formData = new FormData(); formData.append('image', file); const data = await request('/blogs/upload', { method: 'POST', headers: { Authorization: `Bearer ${token()}` }, body: formData }); return data.url; }
let currentPage = 1;
...
$('login-form').addEventListener('submit', async event => { event.preventDefault(); try { const data = await request('/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: $('password').value }) }); sessionStorage.setItem(tokenKey, data.token); $('password').value = ''; showAdmin(true); await Promise.all([loadCategories(), loadArticles()]); } catch (error) { message(error.message, true); } });
...
$('category-form').addEventListener('submit', async event => { event.preventDefault(); const name = $('new-category').value.trim(); if (!name) return; try { await request('/blogs/categories', { method: 'POST', headers: headers(), body: JSON.stringify({ name }) }); $('new-category').value = ''; await loadCategories(); message('Category added.'); } catch (error) { message(error.message, true); } });
$('blog-form').addEventListener('submit', async event => { event.preventDefault(); const id = $('blog-id').value; try { const payload = { title: $('title').value, category: $('category').value, excerpt: $('excerpt').value, content: $('content').value, tags: $('tags').value.split(',').map(tag => tag.trim()).filter(Boolean), coverImage: await uploadCoverImage(), githubUrl: $('githubUrl').value || undefined, deployedUrl: $('deployedUrl').value || undefined, published: $('published').checked }; await request(id ? `/blogs/${id}` : '/blogs', { method: id ? 'PUT' : 'POST', headers: headers(), body: JSON.stringify(payload) }); resetForm(); await loadArticles(); message('Article saved.'); } catch (error) { $('form-message').textContent = error.message; } });
if (token()) { showAdmin(true); Promise.all([loadCategories(), loadArticles()]).catch(error => { sessionStorage.removeItem(tokenKey); showAdmin(false); message(error.message, true); }); }
