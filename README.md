...
`MONGODB_URI`, `ADMIN_PASSWORD_HASH`, and `ADMIN_JWT_SECRET` are server-only. `PORT` is optional locally and should normally be omitted on Railway because Railway provides it. `CLIENT_URL` should be the exact deployed portfolio origin, without a trailing slash. Do not add frontend variables such as `REACT_APP_API_URL` to this backend.
