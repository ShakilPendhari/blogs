...
// Usage: npm run create-blog -- --title "Title" --category React.js --excerpt "Summary" --content "# Markdown" --tags react,api --coverImage https://... --published true
...
async function createBlog() {
  ...
  const blog = await Blog.create({ title: args.title, category: args.category || 'Other', excerpt: args.excerpt, content: args.content, tags: args.tags ? args.tags.split(',').map(tag => tag.trim()).filter(Boolean) : [], coverImage: args.coverImage || undefined, published: args.published === 'true' });
